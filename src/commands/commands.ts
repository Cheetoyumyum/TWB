...
export class CommandHandler {
  ...
  async handleCommand(username: string, command: string, args: string[]): Promise<string | null> {
  ...
  private handleDepositCommand(username: string, args: string[]): string {
    if (args.length > 0) {
  ...
  private handleBuyCommand(username: string, args: string[]): string {
    if (args.length < 1) {
  ...
  private handleGivePoints(username: string, args: string[]): string {
    if (args.length < 2) {
