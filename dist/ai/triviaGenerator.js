...
class TriviaGenerator {
    ...
    async generateTrivia(exclusions = [], categoryHint) {
...
- Do NOT include explanations or extra text—output ONLY the JSON.`;
    ...
    containsForbiddenPair(question, answers) {
        ...
        if (lowerQuestion.includes('ocarina of time')) {
            ...
            if (lowerQuestion.includes('ganondorf') && lowerQuestion.includes('steed')) {
                const hasPhantomGanon = answers.some((ans) => ans.includes('phantom ganon') || ans.includes('phantom'));
                if (!hasPhantomGanon) {
                    return true;
                }
            }
